...
def layer_segmentation(image, real_frame_dis):
    ...
    """
    # Resize image
    image = ResizeWithAspectRatio(image, width= 768)

    # Extract from background using AI
    img = rembg.remove(image)
    image = cv2.cvtColor(img, cv2.COLOR_BGRA2BGR)
    # cv2.imshow("rembg", image)
    # cv2.waitKey(0)
    ...
    """
    # choose to include bianry method or not
    ...
    for i in range(len(radii)):
        ...
        layer_string = "Edge {:.0f} radius = {:.2f}mm".format(i+1, real_radii[i]*1000)
