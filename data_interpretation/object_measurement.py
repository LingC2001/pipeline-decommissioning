...
def findBestContour(closed_binary):
    ...
    if len(sorted_contours) > 0:
        best_fit_contour = sorted_contours[0]
    else:
        best_fit_contour = []
...
def extractPipeAI(image):

    


    # Extract pipe form background
    ...
    mask = cv2.inRange(hsv, (10, 93, 0), (45, 255, 255))
    ...
    # Getting smooth binary image
    grey = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    blur = cv2.bilateralFilter(grey, 5, 75, 75)
    binary = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY_INV)[1]
    ...
    pipe_contour = findBestContour(close)
    
    """
    # Extract contour without using findBestContour Function:

    canny = cv2.Canny(close, 0, 0)
    contours, hierarchy = cv2.findContours(canny, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    contours = sorted(contours, key=cv2.contourArea, reverse=True)
    pipe_contour = contours[0]
    """


    return image, pipe_contour

def drawBox(image, contour, each_pixel_dis, cam_dis, very_long = False, is_horizontal = None):
    """
    ...
    # overwriting length/width decision if needed
    if is_horizontal != None:
        dx_01 = abs(box2[0][0] - box2[1][0])
        dx_03 = abs(box2[0][0] - box2[3][0])
        dy_01 = abs(box2[0][1] - box2[1][1])
        dy_03 = abs(box2[0][1] - box2[3][1])
        if is_horizontal:
            if dx_01 > dx_03:
                long_edge_is_01 = True
                length = x
    ...
    if very_long:
        obj_length = each_pixel_dis*real_length * 1000 #mm
    else:
        obj_length = each_pixel_dis*length * 1000 #mm
    obj_width = each_pixel_dis*width * 1000 #mm

    dimensions_string = "length={:.2f}mm, diameter={:.2f}mm".format(obj_length, obj_width)
    cv2.putText(image, dimensions_string, tuple(box2[0] + [-box2[0][0]+30, -10]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
    cv2.drawContours(image, [box2], 0, (0, 0, 255))
...
def object_measurement(image, real_frame_width, cam_dis, background=True, very_long = False, is_horizontal = None):
    """
    ...
    """
    mask_image, pipe_contour = extractPipeAI(image)

    if pipe_contour != []:

        if background:
            output_image, obj_length, obj_width = drawBox(image, pipe_contour, each_pixel_dis, cam_dis, very_long, is_horizontal)
        else:
            output_image, obj_length, obj_width = drawBox(mask_image, pipe_contour, each_pixel_dis, cam_dis, very_long, is_horizontal)
    else:
        return image, 0, 0

    # # Showing results
    # print('The length of the pipe is: %.4f mm'%(obj_length))
    # print('The diameter of the pipe is: %.4f mm'%(obj_width))
