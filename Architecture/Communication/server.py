...
MESSAGE_DEST = '../Databases/messages/'
    ...
    def service_connection(self, key, mask):
        ...
        if mask & selectors.EVENT_READ:
            ...
            if recv_data and curr_conn.header:
                ...
                if curr_conn.header['mode']== 'file' :
                    f_name = curr_conn.header['filename'].split('/')[-1]
                    write_file (FILE_DEST + f_name, recv_data)

                elif curr_conn.header['mode'] == 'message' :
                    store_message (MESSAGE_DEST+'messages.txt', recv_data)
        ...
        #     if curr_conn.header:
        #         #print(f"acknowledging header")
        #         ack = 'received header'.encode()
        #         sent = sock.send(ack)  # Should be ready to write
